> ⚠️ **已归档（{date}）· 上游历史文档，勿作为本项目现状依据**
>
> 本文原属上游 quick-login 项目（随源码同步进本仓库），描述的是**上游 v3.13.2 及更早**的形态。
> 本项目（Akso Workbench）已在其上叠加私有改造，**权威文档请看根侧 `docs/EXTENSION-PLANE.md`**。
> 保留原文仅为追溯上游设计意图与取证结论（尤其是六平面隔离的攻防过程）。
>
> 上游文档索引见 `docs/archive/README.md`。

---

# QuickLogin 项目现状快照（2026-08-29 · v3.9.2）

> ⚠️ **库内状态（2026-09-10 更新）**：本文为上游历史快照。库内副本已同步至
> **上游 v3.13.2**，并叠加 akso-mogul 私有改造：manifest（host_permissions:18765、
> alarms 权限、quick-wheel 热键 Ctrl+Shift+Q 让位桌面壳）、新增
> `src/background/sync.ts` 桌面同步桥 + `src/background/account-wheel.ts` 轮盘抽取、
> service-worker 挂载同步桥、parallel.html 隐藏账号增删改区块（数据面归桌面端）、
> wheel-overlay 双 interval 泄漏修复。上游仓库无 sync.ts，纯浏览器形态。
